...
class KioskApiException implements Exception {
  KioskApiException(this.message, {this.isConnectionError = false, this.tokenInvalido = false});
...
class KioskEquipeItem {
  const KioskEquipeItem({required this.funcionarioId, required this.nome, required this.faceVersion});
...
class KioskIdentificacao {
  const KioskIdentificacao({required this.funcionarioId, required this.nome, required this.proximoSentido});

  final int funcionarioId;
  final String nome;
  final String proximoSentido; // ENTRADA ou SAIDA

  factory KioskIdentificacao.fromJson(Map<String, dynamic> j) => KioskIdentificacao(
        funcionarioId: j['funcionarioId'] as int,
        nome: j['nome'] as String,
...
class KioskRecenteItem {
  const KioskRecenteItem({required this.funcionarioId, required this.nome, required this.sentido, required this.dataHora});
...
class KioskApi {
  ...
  Future<List<KioskEquipeItem>> equipe(String token) async {
    try {
      final res = await _api.get<List<dynamic>>('/api/kiosk/equipe', query: {'token': token});
      final data = res.data ?? const [];
      return data.map((e) => KioskEquipeItem.fromJson(e as Map<String, dynamic>)).toList();
  ...
  Future<KioskIdentificacao> identificar(String token, String tipo, String valor) async {
    try {
  ...
  /// reconhecido).
  Future<KioskRegistro> registrar(String token, String tipo, String valor) async {
    try {
      final res = await _api.post<Map<String, dynamic>>(
        '/api/kiosk/registrar',
        data: {'token': token, 'tipo': tipo, 'valor': valor},
  ...
  Future<KioskRegistro> registrarPorFace(String token, int funcionarioId, String fotoEvidenciaDataUri) async {
    try {
      final res = await _api.post<Map<String, dynamic>>(
        '/api/kiosk/face/registrar',
        data: {'token': token, 'funcionarioId': funcionarioId, 'fotoEvidenciaBase64': fotoEvidenciaDataUri},
      );
      final data = res.data;
      if (data == null) throw KioskApiException('Resposta vazia do servidor.');
      return KioskRegistro.fromJson(data);
  ...
  Future<List<KioskRecenteItem>> recentes(String token) async {
    try {
      final res = await _api.get<List<dynamic>>('/api/kiosk/recentes', query: {'token': token});
      final data = res.data ?? const [];
      return data.map((e) => KioskRecenteItem.fromJson(e as Map<String, dynamic>)).toList();
    } on DioException catch (e) {
  ...
  KioskApiException _mapError(DioException e, {required String recurso}) {
    ...
    final mensagem = (detalhes is String && detalhes.isNotEmpty) ? detalhes : humanizeDio(e, recurso: recurso);
    return KioskApiException(mensagem, isConnectionError: isConnectionError, tokenInvalido: status == 403);
  }
}
